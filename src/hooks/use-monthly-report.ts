...
import { getMonthlySalesReport } from "@/services/reports/report-service";
import { useReportsStore } from "@/store/reports-store";
import type { MonthlySalesReport } from "@/types/reports";
import { formatFirebaseError } from "@/utils/firebase-error";

export const useMonthlyReport = (employeeId?: string) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await getMonthlySalesReport(firebaseDb(), dateRange, employeeId);
      setReport(result);
    } catch (err) {
